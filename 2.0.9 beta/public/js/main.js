...
// Funkcja zapisywania adresu IP w config.json
...
function loadData() {
    fetch(`/load`)
        ...
        .then(data => {
            ...
            // Sprawdzenie unikalności identyfikatorów scen
            const sceneIds = scenes.map(scene => scene.id);
            const uniqueSceneIds = new Set(sceneIds);
            if (sceneIds.length !== uniqueSceneIds.size) {
                console.warn('Wykryto duplikaty identyfikatorów scen. Może to prowadzić do nieprawidłowego działania aplikacji.');
            }
...
function copyScene(index) {
    ...
    copiedScene.id = `scene-${uuidv4()}`; // Użycie uuidv4() do generowania unikalnego ID
    ...
    scenes.forEach((scene, index) => {
        ...
        if (scene.type === 'regular') {
            ...
            sceneContent = `
                ...
                <span class="scene-content">Cues: ${scene.cues ? scene.cues.length : 0}</span>
            `;
        }
...
function saveData() {
    fetch('/save', { // Zmiana endpointu na /save
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(response => response.json())
    .then(data => {
        console.log('Dane sesji zapisane pomyślnie.');
    })
    .catch(error => {
...
// Funkcja zapisywania adresu IP do serwera
