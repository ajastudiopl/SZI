    ...
    scenes.forEach((scene, index) => {
        ...
        if (scene.type === 'regular') {
            ...
            sceneContent = `
                ...
                <strong>
                    <span class="scene-order">${scene.order || index + 1}</span>.
            ...
            sceneContent = `
                ...
                <strong>
                    <span class="scene-order">${scene.order || index + 1}</span>.
                    <span class="scene-title">${scene.title} (Timeline)</span>
                    <button class="toggle-cues" onclick="toggleCues(${index})">Cues</button>
                </strong><br>
                <div id="cue-list-${index}" class="cue-list" style="display: none; margin-left: 20px;">
                    <!-- Lista Cue zostanie dodana dynamicznie -->
                </div>
            `;
        }
        sceneDiv.innerHTML = sceneContent;
...
//rozwijanie i zwijanie listy cue
function toggleCues(index) {
    const cueListDiv = document.getElementById(`cue-list-${index}`);
    if (cueListDiv.style.display === 'none' || cueListDiv.style.display === '') {
        cueListDiv.style.display = 'block';
        renderCueList(index);
    } else {
        cueListDiv.style.display = 'none';
    }
}
//wygenerowana lista cue 
function renderCueList(sceneIndex) {
    const scene = scenes[sceneIndex];
    const cueListDiv = document.getElementById(`cue-list-${sceneIndex}`);
    cueListDiv.innerHTML = '';

    if (scene.cues && scene.cues.length > 0) {
        scene.cues.forEach((cue, cueIndex) => {
            const cueDiv = document.createElement('div');
            cueDiv.classList.add('cue-item');
            cueDiv.innerHTML = `
                <strong>Cue ${cueIndex + 1}: ${cue.name}</strong> (czas: ${formatTimeWithMillis(cue.time)})
                <button onclick="editCue(${sceneIndex}, ${cueIndex})">Edytuj Cue</button>
            `;
            cueListDiv.appendChild(cueDiv);
        });
    } else {
        cueListDiv.innerHTML = '<em>Brak Cue w tej scenie.</em>';
    }
}
//time
function formatTimeWithMillis(time) {
    ...
    return `${minutes}:${seconds < 10 ? '0' + seconds : seconds}:${millis < 10 ? '0' + millis : millis}`;
}
function editCue(sceneIndex, cueIndex) {
    const scene = scenes[sceneIndex];
    const cue = scene.cues[cueIndex];

    // Możesz tutaj otworzyć modal do edycji Cue
    // lub wyświetlić informacje o Cue w inny sposób

    alert(`Edycja Cue: ${cue.name}\nTo jest miejsce, gdzie możesz zaimplementować edycję Cue.`);
}
...
function editScene(index) {
    ...
    alert('Edycja scen typu "timeline" nie jest dostępna w tym trybie. Proszę użyć timeline.html do edycji tej sceny.');
    return;
}
...
function saveContentToMany() {
    ...
    fetch('/save-scenes', {
        ...
        body: JSON.stringify({ scenes: scenes, users: users })
    })
    .then(response => response.json())
    .then(data => {
        console.log('Sceny zapisane pomyślnie.');
    })
    .catch(error => {
